...
# Run the MCP server
run:
	@echo "Running Historic Environment Records MCP server..."
	@if command -v uv >/dev/null 2>&1; then \
		PYTHONPATH=src uv run python -m chuk_mcp_her.server; \
	else \
		PYTHONPATH=src python3 -m chuk_mcp_her.server; \
	fi
...
docker-build:
	...
	docker build -t chuk-mcp-her:latest .
	@echo "Docker image built: chuk-mcp-her:latest"
...
publish-test: build
	...
	echo ""; \
	echo "Install with:"; \
	echo "  pip install --index-url https://test.pypi.org/simple/ chuk-mcp-her==$$version"
...
publish-manual: build
	...
	echo "Install with: pip install chuk-mcp-her==$$version"
